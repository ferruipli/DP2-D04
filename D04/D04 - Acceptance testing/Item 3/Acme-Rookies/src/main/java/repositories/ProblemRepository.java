...
public interface ProblemRepository extends JpaRepository<Problem, Integer> {
	...
	@Query("select distinct p from Application a join a.problem p where a.position.id=?1 and (a.rookie.id = ?2 or a.status = 'ACCEPTED')")
	Collection<Problem> problemsWithAcceptedApplicationWithOwnApplication(int idPosition, int idRookie);
